...
interface NotificationApi {
    ...
    @ApiResponses(
        value = [ApiResponse(code = 200, message = "OK", response = NotificationDto::class, responseContainer = "List")]
    )
    @RequestMapping(
    ...
    fun saveNotificationReadStatus(
        @ApiParam(value = "ID of the notification", required = true) @PathVariable("notificationId") notificationId: java.util.UUID,
        @ApiParam(value = "", required = true) @Valid @RequestBody body: kotlin.Boolean
    ): ResponseEntity<Unit> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }
}
