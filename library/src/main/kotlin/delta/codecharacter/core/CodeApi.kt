...
interface CodeApi {
    ...
    fun createCodeRevision(
        @ApiParam(value = "", required = true) @Valid @RequestBody createCodeRevisionRequestDto: CreateCodeRevisionRequestDto
    ): ResponseEntity<Unit> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }

    @ApiOperation(
        value = "Get code revision by ID",
        nickname = "getCodeRevisionById",
        notes = "Get code revision by ID",
        response = CodeRevisionDto::class,
        authorizations = [Authorization(value = "http-bearer")]
    )
    @ApiResponses(
        value = [ApiResponse(code = 200, message = "OK", response = CodeRevisionDto::class), ApiResponse(code = 401, message = "Unauthorized"), ApiResponse(code = 404, message = "Not Found")]
    )
    @RequestMapping(
        method = [RequestMethod.GET],
        value = ["/user/code/revisions/{revisionId}"],
        produces = ["application/json"]
    )
    fun getCodeRevisionById(
        @ApiParam(value = "ID of the code revision", required = true) @PathVariable("revisionId") revisionId: java.util.UUID
    ): ResponseEntity<CodeRevisionDto> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }
    ...
    @ApiResponses(
        value = [ApiResponse(code = 200, message = "OK", response = CodeRevisionDto::class, responseContainer = "List"), ApiResponse(code = 401, message = "Unauthorized")]
    )
    @RequestMapping(
        method = [RequestMethod.GET],
        value = ["/user/code/revisions"],
    ...
    @ApiResponses(
        value = [ApiResponse(code = 200, message = "OK", response = CodeDto::class), ApiResponse(code = 401, message = "Unauthorized")]
    )
    @RequestMapping(
        method = [RequestMethod.GET],
    ...
    fun updateLatestCode(
        @ApiParam(value = "", required = true) @Valid @RequestBody updateLatestCodeRequestDto: UpdateLatestCodeRequestDto
    ): ResponseEntity<Unit> {
        return ResponseEntity(HttpStatus.NOT_IMPLEMENTED)
    }
}
